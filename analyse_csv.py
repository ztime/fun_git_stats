...
def main():
    ...
    parser.add_argument(
        '--csv',
        help='CSV with stats to load'
    )
    parser.add_argument(
        '--author',
        help='Author to analyse'
    )
    parser.add_argument(
        '--save-plots',
    ...
    authors_ranked_by_commits = commits_df.groupby('author')['message'].count().sort_values(ascending=False)
    if args.verbose:
    ...
    commits_totals['insertion_deletion_ratio'] = commits_totals['stats_insertions'] / commits_totals['stats_deletions']
    commits_totals['insertion_deletion_ratio'] = commits_totals['insertion_deletion_ratio'].replace([float('inf')], 0)
    if args.verbose:
        print_dataframe(commits_totals, "Authors ranked by totals")
    totals_columns = ['message_length', 'stats_insertions', 'stats_deletions', 'stats_lines', 'insertion_deletion_ratio']
    for column in totals_columns:
        author_rank, author_value = get_rank_and_value(args.author, commits_totals, column, ascending=False)
        stats.append(f"Author {args.author} is ranked {author_rank} in totals for {column} with a value of {author_value:.2f}")

    commits_mean = commits_df.groupby('author').mean()
    if args.verbose:
        print_dataframe(commits_mean, "Authors ranked by means")
    means_columns = ['message_length', 'stats_insertions', 'stats_deletions', 'stats_lines', 'neg', 'neu', 'pos', 'compound']
    for column in means_columns:
        author_rank, author_value = get_rank_and_value(args.author, commits_mean, column, ascending=False)
        stats.append(f"Author {args.author} is ranked {author_rank} in means for {column} with a value of {author_value:.2f}")
    ...
    hour_distplot = ff.create_distplot(
        [commits_df[commits_df['is_author']]['hour'], commits_df[~commits_df['is_author']]['hour']],
        [args.author, 'Others'],
        show_rug=False,
    )
    hour_distplot.update_layout(legend=dict(
        orientation="h",
        yanchor="bottom",
        y=1.02,
        xanchor="right",
